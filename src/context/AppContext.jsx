...
export function AppProvider({ children }) {
  ...
  const [activeScreen, setActiveScreen] = useState("home"); // 'home' | 'search' | 'store'
  const [searchQuery, setSearchQuery] = useState("Tomato");
  const [selectedStoreId, setSelectedStoreId] = useState(null);
  const [liveStreamStore, setLiveStreamStore] = useState(null);
  ...
  const openLiveStream = (store) => {
    setLiveStreamStore(store);
  };
   ...
   */
  const processCheckoutOrder = async (completedPayment = null) => {
    if (cart.length === 0) {
    ...
    let paymentResult = completedPayment;
    if (!paymentResult || !paymentResult.success) {
      paymentResult = await defaultPaymentService.processPayment({
        amount: grandTotal,
        currency: "INR",
