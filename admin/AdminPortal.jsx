...
import CameraPublisher from "../CameraPublisher.jsx";
  ...
  return (
    <div className="min-h-screen bg-base text-ink flex flex-col antialiased">
      ...
      <header className="bg-surface border-b-3 border-ink sticky top-0 z-30 shadow-brutal-sm">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center gap-2 overflow-x-auto scrollbar-none py-2 border-t border-ink/10">
          {[
            ...
            { id: "publisher", label: "Webcam CCTV Publisher", icon: Radio },
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        ...
        {/* WEBCAM PUBLISHER TAB */}
        {activeTab === "publisher" && (
          <div className="space-y-6">
            <CameraPublisher
              initialStreamId={currentStore?.cameraStreamId || "store1"}
              storeName={adminUser?.storeName || currentStore?.name || "Store #1 Camera"}
            />
          </div>
        )}
        ...
        {activeTab === "overview" && (
          <div className="space-y-6">
            ...
            {/* Live Store Shelf Camera Stream Feed (WebRTC Webcam Engine) */}
            <CameraPublisher
              initialStreamId={currentStore?.cameraStreamId || "store1"}
              storeName={adminUser?.storeName || currentStore?.name || "Store #1 Camera"}
            />
                        ...
                        <tr key={log.id} className="hover:bg-base/50 transition">
                          ...
                          <td className="py-2.5 px-3">
                            {log.verdict === "verified" ? (
                              ...
                              <span className="px-2 py-0.5 rounded bg-purple-100 text-purple-950 border border-purple-700 font-extrabold">
                                AI FAKE
                              </span>
                            ) : (
